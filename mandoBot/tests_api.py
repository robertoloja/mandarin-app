...
from ninja.testing import TestAsyncClient  # noqa: E402
...
class SegmentationAPITest(TestCase):
    ...
    async def test_empty_request(self):
        response = await self.client.post("/segment?data=")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, self.emptyResponse)

    async def test_no_mandarin_request(self):
        request_data = "english"
        response = await self.client.post(f"/segment?data={request_data}")
    ...
    async def test_includes_punctuation(self):
        request_data = "《北京城市总体规划（2016年—2035年）》"
        response = await self.client.post(f"/segment?data={request_data}")
        pass  # TODO: Finish

    async def test_dictionary(self):
        test_sentences = [
    ...
    async def test_mandarin_is_segmented_traditional(self):
    ...
    async def test_mandarin_is_segmented_simplified(self):
